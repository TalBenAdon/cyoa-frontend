...
export default function ToolItemSlot({ itemName }: ToolItemSlotProps) {
    const isEmpty = !itemName
    return (
        <div className="flex items-center text-[12px] lg:text-sm justify-between">
            <div className="flex items-center">
                <PlaceHolderIcon className="lg:w-[14px] lg:h-[14px]" />
                <p className={`${isEmpty ? 'text-text-base' : ''}`}>
                    {isEmpty ? '[empty slot]' : itemName}
                </p>
            </div>
            <button>
                <RemoveIcon className="lg:w-[12px] lg:h-[12px] hover:text-add-remove-hover" />
            </button>
        </div>
    )
}